...
| A watermark left inside a record never seals half of one | a log shortened while its sealed-bytes mark stayed put -- what an interrupted `forget` leaves -- publishes whole records from the start of the one the mark landed in, and the command recorded afterwards reaches the peer |
